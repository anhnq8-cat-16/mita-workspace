...
- **Duyệt:** trưởng nhóm Marketing (thư viện chung), trưởng nhóm Sale (tài liệu nội bộ Sales), quản lý, admin – tab *Chờ duyệt*: chọn thư mục đích (đã gợi ý theo sản phẩm/loại, xem trước đường dẫn) → *Duyệt*; hoặc *Từ chối* kèm lý do (người tải lên nhận thông báo). Đổi team được duyệt: `library_approver_teams`. Trưởng nhóm không tự duyệt file của chính mình; **quản lý/admin thì duyệt được cả file mình tải lên**.

# Deploy lên Cloudflare Pages (`work.[DOMAIN]`)
...
## 2. Gắn tên miền `work.[DOMAIN]`
1. Trong project Pages → tab **Custom domains → Set up a custom domain** → nhập `work.[DOMAIN]` → **Continue**.
...
4. Nhớ thêm `https://work.[DOMAIN]` vào Google OAuth Client (**Authorized JavaScript origins**) và `https://work.[DOMAIN]/auth/callback` vào Supabase **Redirect URLs**.
